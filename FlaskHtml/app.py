...
def index():
    ...
    for domain, lb_info in data.items():
        ...
        for lb_ip, as_info in lb_info.items():
            if lb_ip != "domain_info" and as_info != "UCB, US":  # Skip 'domain_info' and 'UCB, US'
                lb_groups[(lb_ip, as_info)][domain_as].add(domain)

    # Flatten the groups to a list of (count, lb_ip, as_info, domain_as, domains) tuples
    flat_groups = []
    for (lb_ip, as_info), domain_groups in lb_groups.items():
        for domain_as, domains in domain_groups.items():
            flat_groups.append((len(domains), lb_ip, as_info, domain_as, ', '.join(domains)))
    ...
    json_data = [{'count': count, 'lb_ip': lb_ip, 'as_info': as_info, 'domain_as': domain_as, 'domains': domains} 
                 for count, lb_ip, as_info, domain_as, domains in sorted_groups[:20]]
